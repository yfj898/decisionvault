...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply governed_adaptive_memory_v6.sql one statement per CockroachDB "
            "transaction so schema changes are visible before dependent steps."
        )
    )
    ...
    statements = _statements(
        (ROOT / "scripts" / "governed_adaptive_memory_v6.sql").read_text(
            encoding="utf-8"
        )
    )
        ...
        with conn.cursor() as cur:
            for index, statement in enumerate(statements, start=1):
                try:
                    ...
                    print(
                        "governed_adaptive_memory_v6_statement_"
                        f"{index}=FAIL:{type(exc).__name__}"
                    )
                    return 2
    finally:
        conn.close()

    print(f"governed_adaptive_memory_v6_statements={len(statements)}")
    print("governed_adaptive_memory_v6_apply=PASS")
    return 0
