...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply memory-quality sampling-bias and retention v10 schema."
    )
    ...
    statements = _statements(
        (ROOT / "scripts" / "memory_quality_sampling_v10.sql").read_text(
            encoding="utf-8"
        )
    )
        ...
        with conn.cursor() as cur:
            for index, statement in enumerate(statements, start=1):
                try:
                    ...
                    print(
                        f"memory_quality_sampling_v10_statement_{index}="
    ...
    print(f"memory_quality_sampling_v10_statements={len(statements)}")
    print("memory_quality_sampling_v10_apply=PASS")
    return 0
