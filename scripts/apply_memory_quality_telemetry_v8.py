...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply append-only memory-quality telemetry v8 schema/grants."
    )
    ...
    statements = _statements(
        (ROOT / "scripts" / "memory_quality_telemetry_v8.sql").read_text(
            encoding="utf-8"
        )
    )
        ...
        with conn.cursor() as cur:
            for index, statement in enumerate(statements, start=1):
                try:
                    ...
                    print(
                        f"memory_quality_telemetry_v8_statement_{index}="
                        f"FAIL:{type(exc).__name__}"
                    )
                    return 2
    finally:
        conn.close()
    print(f"memory_quality_telemetry_v8_statements={len(statements)}")
    print("memory_quality_telemetry_v8_apply=PASS")
    return 0
