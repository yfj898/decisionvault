...
<body><main>
  ...
  <h1>DecisionVault</h1>
  <p class="lede">Persistent decision memory for autonomous agents. This live demo proves that the same agent repeats an inferior action with memory disabled, then changes behavior after recalling a prior outcome from CockroachDB.</p>
  ...
  <section class="grid">
    <div class="card"><h2>Run the live causal proof</h2><p class="muted">The protected demo creates a temporary failed episode, runs Memory OFF and Memory ON against the same similar situation, returns the comparison, then deletes the temporary scope.</p>
    ...
    <div class="card"><h2>Authority boundary</h2><div class="flow"><div class="step">1 · Recall outcome evidence from CockroachDB</div><div class="step">2 · Deterministic policy commits the strategy</div><div class="step">3 · NVIDIA may explain the committed strategy only</div><div class="step">4 · Temporary demo memory is deleted after the proof</div></div><p class="foot">The model cannot select a different strategy. Provider failure never blocks or changes the deterministic memory-aware decision.</p></div>
  </section>
...
function render(prefix,v){$(prefix+'Strategy').textContent=v.strategy||'—';$(prefix+'Kv').textContent=`memory_influenced=${v.memory_influenced} · recalled=${(v.recalled_episode_ids||[]).length} · provider=${v.model_provider||'none'}`;$(prefix+'Explanation').textContent=v.model_explanation||''}
run.addEventListener('click',async()=>{const token=$('token').value.trim();if(!token){$('status').textContent='Enter the demo access token first.';return}run.disabled=true;$('status').textContent='Running Memory OFF → seed outcome → Memory ON → cleanup…';const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),28000);try{const r=await fetch('/demo',{method:'POST',headers:{'content-type':'application/json','x-decisionvault-token':token},body:'{}',signal:controller.signal}),j=await r.json();if(!r.ok)throw new Error(j.error==='unauthorized'?'Invalid demo token.':`Demo failed (${r.status}).`);render('off',j.memory_off);render('on',j.memory_on);const changed=j.memory_off.strategy!==j.memory_on.strategy&&j.memory_on.memory_influenced===true;$('delta').style.display='block';$('delta').textContent=changed?`PASS · Persistent memory changed behavior: ${j.memory_off.strategy} → ${j.memory_on.strategy}. Temporary scope cleaned=${j.cleaned}.`:'The run completed, but the expected causal strategy change was not observed.';$('status').textContent='Live proof completed.'}catch(e){$('status').textContent=e.name==='AbortError'?'Demo timed out. Try again.':e.message}finally{clearTimeout(timer);run.disabled=false}});health();
</script></body></html>"""
